...
export default {
  ...
  methods: {
    renderIcon: function (h, icon, key, theme) {
      ...
      return !icon || icon == "none"
        ? null
        : h(Icon, { props: { type: icon, theme } });
    },
    renderBadge: function (h, menu) {
      if (menu.meta.count && menu.meta.count != 0) {
        return h(Badge, { props: { count: menu.meta.count } }, [
          this.$t(getI18nKey(menu.fullPath)),
        ]);
      } else {
        return this.$t(getI18nKey(menu.fullPath));
      }
    },
      ...
      return h(Item, { key: menu.fullPath }, [
        h(tag, config, [
          this.renderIcon(
            ...
            menu.fullPath,
            menu.meta ? menu.meta.theme : null
          ),
          this.renderBadge(h, menu),
        ]),
      ]);
    },
