      ...
      <div class="eip-icon-picker ant-dropdown-trigger">
        <span class="ant-input-affix-wrapper">
          <span class="ant-input-prefix" v-if="icon.name">
            <a-icon slot="prefix" :type="icon.name" :theme="icon.theme" />
          </span>
          <input placeholder="请选择图标" readonly="" v-model="icon.name" type="text" style="cursor: pointer"
            class="ant-input" />
        ...
        <div class="ant-tabs-line ant-tabs ant-tabs-top">
          <a-tabs default-active-key="outline" @change="tabChange">
            ...
            <div class="eip-icon-picker-search" slot="tabBarExtraContent">
              <a-input-search allow-clear v-model="key" size="small" placeholder="关键字搜索图标" style="width: 150px"
                @change="keySearch" @search="keySearch" />
            </div>
          </a-tabs>
        </div>
...
.eip-icon-picker-popper .ant-tabs-tab {
...
.eip-icon-picker-popper .ant-card-body {
  padding: 0;
}
