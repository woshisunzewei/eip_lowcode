  ...
  <div class="side-setting">
    <setting-item :title="$t('theme.title')">
      <img-checkbox-group @change="(values) => setTheme({ ...theme, mode: values[0] })" :default-values="[theme.mode]">
        <img-checkbox :title="$t('theme.dark')"
          img="https://gw.alipayobjects.com/zos/rmsportal/LCkqqYNmvBEbokSDscrm.svg" value="dark" />
        <img-checkbox :title="$t('theme.light')"
          img="https://gw.alipayobjects.com/zos/rmsportal/jpRkZQMyYRryryPNtyIC.svg" value="light" />
    ...
    <setting-item :title="$t('theme.color')">
      <color-checkbox-group @change="(values, colors) => setTheme({ ...theme, color: colors[0] })"
    ...
    <setting-item :title="$t('navigate.title')">
      <img-checkbox-group @change="(values) => setLayout(values[0])" :default-values="[layout]">
        <img-checkbox :title="$t('navigate.side')" img="/images/svg/JopDzEhOqwOjeNTXkoje.svg" value="side" />
        <img-checkbox :title="$t('navigate.head')" img="/images/svg/KDNDBbriJhLwuqMoxcAr.svg" value="head" />
        <img-checkbox :title="$t('navigate.mix')" img="/images/svg/LCkqqYNmvBEbokSDscrm.svg" value="mix" />
      </img-checkbox-group>
    </setting-item>
    <setting-item>
      <a-list :split="false">
        <a-list-item>
          ...
          <a-select :getPopupContainer="getPopupContainer" :value="pageWidth" @change="setPageWidth" class="select-item"
            size="small" slot="actions">
    ...
    <setting-item :title="$t('animate.title')">
      <a-list :split="false">
        <a-list-item>
          {{ $t("animate.disable") }}
          <a-switch :checked="animate.disabled" slot="actions" size="small"
            @change="(val) => setAnimate({ ...animate, disabled: val })" />
        </a-list-item>
        <a-list-item>
          {{ $t("animate.effect") }}
          <a-select :value="animate.name" :getPopupContainer="getPopupContainer"
            @change="(val) => setAnimate({ ...animate, name: val })" class="select-item" size="small" slot="actions">
            <a-select-option :key="index" :value="item.name" v-for="(item, index) in animates">{{ item.alias
              }}</a-select-option>
          </a-select>
        </a-list-item>
        <a-list-item>
          {{ $t("animate.direction") }}
          <a-select :value="animate.direction" :getPopupContainer="getPopupContainer"
            @change="(val) => setAnimate({ ...animate, direction: val })" class="select-item" size="small"
            slot="actions">
...
export default {
  ...
  methods: {
    ...
    saveSetting() {
      const closeMessage = this.$message.success("保存成功");
