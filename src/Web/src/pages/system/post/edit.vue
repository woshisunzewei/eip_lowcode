  ...
  <a-modal width="600px" v-drag :destroyOnClose="true" :maskClosable="false" :visible="visible"
    ...
    <template slot="footer">
      <div class="flex justify-between align-center">
        <div>
          <a-checkbox v-if="!postId" v-model="save.retain">
            继续创建时，保留本次提交内容
          </a-checkbox>
        </div>

        <a-space>
          <a-button v-if="!postId" key="submit" @click="saveContinue" :loading="loading"><a-icon
              type="save" />提交并继续创建</a-button>
          <a-button v-if="postId" @click="cancel"><a-icon type="close" />关闭</a-button>
...
<script>
import { save, findById } from "@/services/system/post/edit";
...
export default {
  ...
  methods: {
    ...
    find() {
      ...
      if (this.postId) {
        that.spinning = true;
        findById(this.postId).then(function (result) {
