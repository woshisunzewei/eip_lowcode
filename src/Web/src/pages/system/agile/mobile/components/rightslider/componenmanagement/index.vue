<template>
  <div class="componenManagement">
    ...
    <vuedraggable v-model="data" v-bind="dragOptions">
      <transition-group>
        <div v-for="(item, ind) in data" :key="item.text + ind"
          :class="item.text == '底部导航' ? 'item delDragitem' : 'item'">
          <p>
            <a-icon type="table" style="margin-right: 15px"></a-icon>
            {{ item.text }}
          </p>
          <a-popconfirm title="您确定要删除该组件吗?" @confirm="onConfirms(ind)">
            <a-icon @click.stop="" style="cursor: pointer" type="delete"></a-icon>
          </a-popconfirm>
        </div>
      </transition-group>
    </vuedraggable>
