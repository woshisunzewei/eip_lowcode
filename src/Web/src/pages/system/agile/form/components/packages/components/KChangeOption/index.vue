    ...
    <a-row v-if="type === 'rules'" :gutter="8">
      <span v-for="(val, index) in value" :key="index">
        <div class="option-change-box" v-if="index !== 0">
          <a-col :span="18"><Input allow-clear v-model="val.message" placeholder="提示信息" /></a-col>
          <a-col :span="18"><Input allow-clear v-model="val.pattern" placeholder="正则表达式">
            <div slot="addonAfter">
              <a-dropdown>
                <a-icon type="down" />
                <a-menu slot="overlay" @click="f => { val.pattern = f.key }">
                  <a-menu-item v-for="(pitem) in eipPattern" :key="pitem.pattern">
                    {{ pitem.title }}
                  </a-menu-item>
                </a-menu>
              </a-dropdown>
