...
<template>
  <div class="operating-area">
    ...
    <div class="left-btn-box">
      ...
      <a-divider v-if="toolbars.includes('save')" type="vertical" />
      <a-tooltip title="预览">
      ...
      <a-tooltip title="导出">
        <a v-if="toolbars.includes('exportJson')" @click="$emit('handleOpenJsonModal')">
          ...
          <span v-if="showToolbarsText">导出</span>
        </a>
      </a-tooltip>
      <Divider type="vertical" />
      ...
      <Divider type="vertical" />
      <a-tooltip title="撤销">
        <a v-if="toolbars.includes('undo')" :class="{ disabled: !(recordList.length > 0) }" @click="$emit('handleUndo')">
          <a-icon type="undo" />
