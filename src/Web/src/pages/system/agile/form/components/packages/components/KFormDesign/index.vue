<template>
  <a-config-provider :locale="locale">
    <div class="eip-form-designer-container" v-if="loadState">
        ...
        <aside class="right">
          <a-tabs :activeKey="activeKey" @change="changeTab" :tabBarStyle="{ margin: 0 }">
            <a-tab-pane :key="1" tab="表单属性设置">
              <formProperties :config="data.config" :previewOptions="previewOptions" :list="data.list" />
            </a-tab-pane>
...
export default {
  ...
  methods: {
    ...
    resetData() {
      this.data = JSON.parse(jsonFormat);
      ...
      const traverse = array => {
        array.forEach(element => {
          if (["grid", "tabs", "selectInputList", "collapse"].includes(element.type)) {
            if (element.type === "selectInputList") {
              fields.push(element);
            }
