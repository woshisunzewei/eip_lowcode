 ...
 * @LastEditTime: 2022-10-28 22:55:07
 -->
<template>
  <component v-if="record.options.noFormItem" v-bind="getComponentProps" @hook:mounted="childMounted"
    :is="componentItem">
  </component>
  <!-- 可隐藏label -->
  <a-form-item v-else :label-col="formConfig.layout === 'horizontal' &&
    isShowLabel(record.options.showLabel)
    ? formConfig.labelLayout === 'flex'
      ? { style: `width:${formConfig.labelWidth}px` }
      : formConfig.labelCol
    : {}
    " :wrapper-col="formConfig.layout === 'horizontal' &&
      isShowLabel(record.options.showLabel)
      ? formConfig.labelLayout === 'flex'
        ? { style: 'width:auto;flex:1' }
        : formConfig.wrapperCol
      : {}
      " :style="formConfig.layout === 'horizontal' &&
      formConfig.labelLayout === 'flex' &&
      isShowLabel(record.options.showLabel)
      ? { display: record.options.hidden && !fromDesign ? 'none' : 'flex' }
      : { display: record.options.hidden && !fromDesign ? 'none' : '' }
      ">
    <span slot="label" v-if="isShowLabel(record.options.showLabel)">
      <a-tooltip>
        <span :style="{
          fontWeight: (formConfig.labelBold || record.options.labelStyle == 'bold') ? 'bold' : '',
          fontSize: record.options.labelSize ? record.options.labelSize + 'px' : '',
          fontStyle: record.options.labelStyle ? record.options.labelStyle : '',
          textDecoration: record.options.labelStyle ? record.options.labelStyle : '',
          fontFamily: record.options.labelFamily ? record.options.labelFamily : '',
          color: record.options.labelColor ? record.options.labelColor : '',
        }" v-text="record.label"></span>
    ...
    <component :is="componentItem" v-bind="getComponentProps" @hook:mounted="childMounted" ref="inputItem"
      :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" @change="handleChange" v-decorator="[
...
export default {
  ...
  computed: {
    ...
    getComponentProps() {
      ...
      if (this.record.type === "cascader" && componentProps.disabled) {
      ...
      if (this.record.type === "treeSelect") {
        componentProps.treeData = !record.options.dynamic
          ...
          : this.dynamicData[record.options.dynamicKey]
            ? this.dynamicData[record.options.dynamicKey]
            : [];
        componentProps.treeNodeFilterProp = "title";
      }
  ...
  methods: {
    ...
    validationSubform() {
      // 验证动态表格
      if (["batch", "selectInputList", "collapse"].includes(this.record.type)) {
