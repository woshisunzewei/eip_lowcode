...
<template>
  <div class="form-panel">
    <p class="hint-text" v-show="data.list.length === 0">
      <eip-empty description="从左侧选择控件添加" />
    </p>
    ...
    >
      <ul>
        ...
        <li @click="handleStyle"><a-icon type="code" />样式设置</li>
      </ul>
    </div>
    ...
    <a-modal
      ...
      :destroyOnClose="true"
      @cancel="style.visible = false"
    >
      <template slot="footer">
        <a-space>
          <a-button @click="style.visible = false"
            ><a-icon type="close" />关闭</a-button
          >
          <a-button key="submit" @click="handleStyleOk" type="primary"
            ><a-icon type="save" />确定</a-button
          >
        </a-space>
      </template>
      <div style="height: 400px">
        <eip-code :value.sync="style.value"></eip-code>
      </div>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleStyle() {
      ...
      this.style.value =
        this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].style;
    },
