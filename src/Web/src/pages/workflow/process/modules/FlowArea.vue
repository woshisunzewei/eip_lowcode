<template>
  <div style="width: 100%; height: 100%; overflow: hidden; position: relative;" @dragover="handleDragover"
    @drop="handleDrop">
    <!--辅助线X-->
  ...
  }" :style="gridStyle" @click.stop="containerHandler" @mousedown="mousedownHandler" @mousemove="mousemoveHandler"
      @mouseup="mouseupHandler" @mousewheel="scaleContainer" @DOMMouseScroll="scaleContainer"
      @contextmenu="showContainerContextMenu">
      <flow-node v-for="node in flowData.nodeList" :key="node.id" :node="node" :flowConfig="flowConfig" :plumb="plumb"
        :select.sync="currentSelect" :selectGroup.sync="currentSelectGroup" :currentTool="currentTool"
        :processInstanceId="processInstanceId" @showNodeContextMenu="showNodeContextMenu" @isMultiple="isMultiple"
        @updateNodePos="updateNodePos" @alignForLine="alignForLine" @hideAlignLine="hideAlignLine">
      </flow-node>
    ...
    <vue-context-menu v-if="!processInstanceId" :contextMenuData="containerContextMenuData" @flowInfo="flowInfo"
      @paste="paste" @selectAll="selectAll" @saveFlow="saveFlow" @verticaLeft="verticaLeft"
      @verticalCenter="verticalCenter" @verticalRight="verticalRight" @levelUp="levelUp" @levelCenter="levelCenter"
      @levelDown="levelDown" @addRemark="addRemark">
    </vue-context-menu>
    <vue-context-menu v-if="!processInstanceId" :contextMenuData="nodeContextMenuData" @copyNode="copyNode"
      @deleteNode="deleteNode" @settingNode="settingNode">
    </vue-context-menu>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteNode() {
      let that = this;
      deleteConfirm(
        "确定删除节点:【" + this.currentSelect.text.text + "】",
        function () {
          ...
          that.flowData.status = flowStatus.LOADING
          ...
          that.flowData.status = flowStatus.CREATE
    ...
    settingNode() {
      this.$emit('settingNode', this.currentSelect)
    ...
    addNewNode(node) {
      ...
      x = nodePos.x
      y = nodePos.y
      let newNode = this.$utils.clone(node, true);
      newNode.id = newNode.type + '-' + utils.getId()
      newNode.height = 50
      if (
        newNode.type === 'start' ||
        newNode.type === 'end' ||
        newNode.type === 'subprocess' ||
        newNode.type === 'fork' ||
        newNode.type === 'join'
      ) {
        newNode.x = x - 25
