<template>
  <a-modal width="600px" v-drag centered :destroyOnClose="true" :maskClosable="false" :visible="visible" :title="title"
      ...
      <a-form-model ref="form" :model="form" :rules="rules" :label-col="config.labelCol"
        ...
        <a-row>
          <a-col>
            ...
            <a-form-model-item label="短信服务商" prop="name">
              <a-select :default-value="0" v-model="form.provide">
                <a-select-option v-for="(item, index) in provides" :key="index" :value="item.key">{{ item.value
                  }}</a-select-option>
              </a-select>
            </a-form-model-item>
            <a-form-model-item label="内容">
              <eip-editor ref="smsEditor" v-if="tinymce.show" v-model="form.content" :height="tinymce.height"
...
export default {
  ...
  methods: {
    ...
    contentOk(value) {
      ...
      this.$refs.smsEditor.insertIndex(html, 0);
    },
    ...
    save() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          that.form.contentTxt = that.$refs.smsEditor.getTxt();
