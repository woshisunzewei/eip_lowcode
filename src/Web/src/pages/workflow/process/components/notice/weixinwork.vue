<template>
  <a-modal width="600px" v-drag centered :destroyOnClose="true" :maskClosable="false" placement="right" :title="title"
    :visible="visible" :bodyStyle="{ padding: '1px' }" @cancel="cancel">
        ...
        <a-row>
          <a-col>
            ...
            <a-form-model-item label="内容">
              <eip-editor ref="officialAccountEditor" v-if="tinymce.show" v-model="form.content"
            ...
            <a-form-model-item label="跳转地址">
              ...
              <a-tag color="pink">
                企业微信跳转地址:<br />
                https://域名/#/pages/wechat/work/jump?userId={当前处理人Id}&taskId={任务Id}&type=1 <a-icon title="点击复制"
                  class="copyUrl"
                  data-clipboard-text="https://域名/#/pages/wechat/work/jump?userId={当前处理人Id}&taskId={任务Id}&type=1"
                  @click="urlCopy()" type="copy"></a-icon>
                <br />
                type:1处理，2详情
              </a-tag>
            </a-form-model-item>
...
export default {
  name: "eip-workflow-notice-wei-xin-work",
  data() {
    return {
      tinymce: {
  ...
  methods: {
    ...
    contentOk(value) {
      ...
      this.$refs.officialAccountEditor.insertIndex(html, 0);
    },
    /**
     *内容选择点击
     */
    ...
    save() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          that.form.contentTxt = that.$refs.officialAccountEditor.getTxt();
