<template>
  <a-modal :zIndex="10001" width="600px" v-drag :destroyOnClose="true" :maskClosable="false" :visible="visible"
    :bodyStyle="{ padding: '1px' }" title="删除任务" @cancel="cancel">
      ...
      <a-spin :spinning="spinning">
        <a-form-model ref="form" :model="form" layout="vertical" :rules="rules">
          <a-row>
            <a-col>
              <a-form-model-item>
                <a-alert message="删除任务" description="删除后的任务将被放入系统回收箱，管理员可以【恢复】或【彻底删除】回收箱中的任务。" type="warning" />
              </a-form-model-item>

              <a-form-model-item label="删除理由" prop="comment">
                <a-input allow-clear v-model="form.comment" type="textarea" placeholder="请输入删除理由" />
              </a-form-model-item>
            </a-col>
          </a-row>
        </a-form-model>
