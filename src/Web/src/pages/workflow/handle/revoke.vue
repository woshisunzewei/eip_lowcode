<template>
  <a-modal :zIndex="10001" width="600px" v-drag :destroyOnClose="true" :maskClosable="false" :visible="visible"
    :bodyStyle="{ padding: '1px' }" title="任务撤销" @cancel="cancel">
      ...
      <a-spin :spinning="spinning">
        <a-form-model ref="form" :model="form" layout="vertical" :rules="rules">
          <a-row>
            <a-col>
              <a-form-model-item>
                <a-alert message="任务撤销" description="使用撤销功能放弃申请。申请撤销后在系统中可见但所有相关审批将被回收，撤销者或管理员可以【恢复】被撤销的申请。"
                  type="warning" />
              </a-form-model-item>

              <a-form-model-item label="撤销理由" prop="comment">
                <a-input allow-clear v-model="form.comment" type="textarea" placeholder="撤销理由" />
              </a-form-model-item>
            </a-col>
          </a-row>
        </a-form-model>
