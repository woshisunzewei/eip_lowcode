            ...
            if (GetMethodAttributes(serviceMethod).FirstOrDefault(x => typeof(EasyCachingEvictAttribute).IsAssignableFrom(x.GetType())) is EasyCachingEvictAttribute attribute && attribute.IsBefore == isBefore)
            {
                try
                {
                    if (attribute.IsAll)
                    {
                        ...
                        var cacheKeyPrefix = attribute.DeleteByCacheKeyPrefix ? attribute.CacheKeyPrefix + ":" : _keyGenerator.GetCacheKeyPrefix(serviceMethod, attribute.CacheKeyPrefix);
                        foreach (var item in cacheKeyPrefix.Split(","))
                        {
                            if (attribute.IsHybridProvider)
                            {
                                _hybridCachingProvider.RemoveByPrefix(item);
                            }
                            else
                            {
                                var _cacheProvider = _cacheProviderFactory.GetCachingProvider(attribute.CacheProviderName ?? _options.Value.CacheProviderName);
                                _cacheProvider.RemoveByPrefix(item);
                            }
                        }
                    }
                    else
                    {
                        ...
                        var cacheKey = attribute.DeleteByCacheKeyPrefix ? attribute.CacheKeyPrefix + ":" : _keyGenerator.GetCacheKey(serviceMethod, invocation.Arguments, attribute.CacheKeyPrefix);
                        foreach (var item in cacheKey.Split(","))
                        {
                            if (attribute.IsHybridProvider)
                            {
