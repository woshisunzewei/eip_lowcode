...
using EIP.Common.Repository.MicroOrm.SqlGenerator.QueryExpressions;
using System;
                        ...
                        else
                        {
                            ...
                            if (Provider == SqlProvider.PostgreSQL && qpExpr.QueryOperator is "NOT IN" or "IN")
                            {
                                if (qpExpr.QueryOperator is "IN")
                                    sqlBuilder.AppendFormat("{0}.{1} = ANY(" + RepositoryUtil.GetSymbol() + "{2})", tableName, columnName, vKey);
                                else
                                    sqlBuilder.AppendFormat("{0}.{1} <> ALL(" + RepositoryUtil.GetSymbol() + "{2})", tableName, columnName, vKey);
                            }
                            else
                                sqlBuilder.AppendFormat("{0}.{1} {2} " + RepositoryUtil.GetSymbol() + "{3}", tableName, columnName, qpExpr.QueryOperator, vKey);
                            ...
                            if (Provider == SqlProvider.Oracle &&
                                conditions[0].Value is IList { Count: 0 } list &&
